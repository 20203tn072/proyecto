...
<nav class="navbar navbar-light" style="background-color: white;">
    ...
    <a class="navbar-brand" href="${context}/views/enlace/inicio.jsp">
...
<nav class="navbar navbar-light" style="background-color: hotpink;" >
    <a class="navbar-brand" href="#">
        <b><font face="Arial Rounded MT" size="4" color="black">ENLACE</font></b>
    </a>
    <a class="navbar-brand" href="${context}/views/enlace/inicio.jsp" style="text-align: right">
        <b><font face="Arial Rounded MT" size="4" color="black">REGRESAR</font></b>
    </a>
</nav>
<br>
<h5>Listar Usuarios | Comite</h5>
<br>
<table class="table">
    ...
    <tbody>
    <c:forEach items="${ listUsers2 }" var="user" varStatus="status">
        <tr>
            <td>${ user.idUser }</td>
            ...
            <td>
                <c:if test="${ user.status == 1 }">
                    <form action="${context}/getUserById2" method="POST" style="display: inline;">
                    ...
                    <button onclick="deleteUser('${ user.idUser }','${ user.nombre } ${ user.lastname }');" type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete"><i class="fas fa-trash"></i> Eliminar</button>
                </c:if>
                <c:if test="${ user.status == 0 }">
                    <button type="button" class="btn btn-outline-info" onclick="details('${ user.nombre }', '${ user.lastname }', '${user.email}', '${user.colonia}', '${user.idRole.nameRole}');" data-bs-toggle="modal" data-bs-target="#details"><i class="fas fa-info-circle"></i> Detalles</button>
                </c:if>
            </td>
        </tr>
    </c:forEach>
...
<div class="modal fade" id="delete" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="${context}/deleteUser2" method="POST">
...
<script>
    function details(nombre, lastname, email, colonia, nameRole){
        document.getElementById("lbl_name").innerHTML=nombre;
        document.getElementById("lbl_lastname").innerHTML=lastname;
        document.getElementById("lbl_colonia").innerHTML=colonia;
        document.getElementById("lbl_role").innerHTML=nameRole;
