...
<br>
<h5>Listar Incidentes | Enlace</h5>
<br>
<table class="table">
    ...
    <c:forEach items="${ listIncidents }" var="incident" varStatus="status">
        <tr>
            ...
            <td>${ incidente.idDepartamento.nombre }</td>
            <td>
            ...
            <td>
                <c:if test="${ incident.status == 1 }">
                    <form action="${context}/getIncidentById" method="POST" style="display: inline;">
                        ...
                        <input type="hidden" name="id" value="${ user.idIncident }">
                        <button type="submit" class="btn btn-outline-primary"><i class="fas fa-edit"></i> Modificar</button>
                    </form>
                    <button onclick="deleteUser('${ incident.idIncident }','${ incident.nombre }');" type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete"><i class="fas fa-trash"></i> Eliminar</button>
                </c:if>
                <c:if test="${ incident.status == 0 }">
                    <button type="button" class="btn btn-outline-info" onclick="details('${ incident.nombre }', '${ incident.descripcion }', '${incident.idUser.nombre}', '${user.idDepartamento.nameRole}');" data-bs-toggle="modal" data-bs-target="#details"><i class="fas fa-info-circle"></i> Detalles</button>
                </c:if>
            </td>
        </tr>
    </c:forEach>
