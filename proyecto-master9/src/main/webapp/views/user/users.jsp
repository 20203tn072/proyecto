    ...
    <c:forEach items="${ listUsers }" var="user" varStatus="status">
        <tr>
            ...
            <td>
                ...
                <c:if test="${ user.status == 0 }">
                    <button type="button" class="btn btn-outline-info" onclick="details('${ user.nombre }', '${ user.lastname }', '${user.email}', '${user.idMunicipio.nombre}', '${user.idRole.nameRole}');" data-bs-toggle="modal" data-bs-target="#details"><i class="fas fa-info-circle"></i> Detalles</button>
                </c:if>
            </td>
        </tr>
    </c:forEach>
...
<script>
    function details(nombre, lastname, email, nombreMun, nameRole){
        document.getElementById("lbl_name").innerHTML=nombre;
        document.getElementById("lbl_lastname").innerHTML=lastname;
        document.getElementById("lbl_municipio").innerHTML=nombreMun;
